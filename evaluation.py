...
def net_render_np(mask_np, ibl_np):
    def to_net_ibl_convert(ibl_np):
        resize_ibl = ibl_np[:,:,0]
        resize_ibl = resize_ibl[:80,:]
        resize_ibl = cv2.resize(ibl_np, (32,16))

        sum_ibl = np.sum(resize_ibl)
        if sum_ibl < 1e-3:
            h,w,c = resize_ibl.shape
            return np.zeros((h,w))
            
        return resize_ibl[:,:,0] * 30.0 / sum_ibl        

    s = time.time()
    net_ibl = to_net_ibl_convert(ibl_np)
    if net_ibl.shape[0] != 5 and net_ibl.shape[1] != 32:
        print('net render ibl is wrong, please check: ', net_ibl.shape)
        return None
